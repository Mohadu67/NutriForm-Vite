...
/**
 * Hook pour gérer toutes les données du Dashboard (adapté du web)
...
export const useDashboardData = (sessions = [], records = []) => {
  ...
  // Normaliser les sessions (différents formats possibles)
  useEffect(() => {
  ...
  const stats = useMemo(() => {
    ...
    const prevWeekStart = new Date(getWeekStart());
    prevWeekStart.setDate(prevWeekStart.getDate() - 7);
    const prevWeekSessions = userSessions.filter((s) => {
      const date = parseDate(s?.date || s?.createdAt || s?.endedAt);
      if (!date) return false;
      return date >= prevWeekStart && date < getWeekStart();
    }).length;

    let sessionsTrend = 'same';
    if (last7Days > prevWeekSessions) sessionsTrend = 'up';
    else if (last7Days < prevWeekSessions) sessionsTrend = 'down';
  ...
  // Points de poids pour graphique (7 derniers)
  ...
  // Calories brulées cette semaine
  const weeklyCalories = useMemo(() => {
    ...
    return userSessions
      ...
      .reduce((acc, s) => acc + (s?.caloriesBurned ?? s?.calories ?? 0), 0);
  }, [userSessions, parseDate]);
  ...
  const sportStats = useMemo(() => {
    ...
    return {
      ...
      hasCardio: stats.run > 0 || stats.bike > 0 || stats.swim > 0 || stats.walk > 0,
    };
  }, [userSessions, getSportType, getEntryDistanceKm]);
